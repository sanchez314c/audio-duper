...
// Neo-Noir Glass Monitor — Frameless transparent window
function createWindow() {
  ...
  // Hide the application menu bar for frameless aesthetic
  ...
  // Dev tools available via Ctrl+Shift+I but not auto-opened
  // To force open: set OPEN_DEVTOOLS=1 environment variable
  if (process.env.OPEN_DEVTOOLS === '1') {
    mainWindow.webContents.openDevTools();
  }

  // Handle external links — protocol validation to prevent arbitrary scheme execution
...
class AudioDedupe {
  ...
  async analyzeDirectory(dirPath) {
    try {
      ...
      return {
        ...
        formats: Array.from(this.stats.byExtension.entries())
          .map(([ext, data]) => ({
            extension: ext,
            count: data.count,
            size: data.size,
  ...
  async scanDirectory(dirPath, progressCallback) {
    try {
      ...
      progressCallback({
        processed: 0,
        total: 1,
        current: 'Discovering audio files...',
      });
      ...
      console.log(
        `🚀 TURBO MODE: Processing with ${maxConcurrency} concurrent operations (ALL ${os.cpus().length} cores + I/O optimization)`
      );
  ...
  async getFingerprint(filePath) {
    try {
      ...
      console.warn(
        `Primary fingerprinting failed for ${filePath}: ${error.message}`
      );
      ...
      try {
        ...
        console.log(
          `Using size+duration fingerprint for ${path.basename(filePath)}: ${sizeFingerprint}`
        );
        ...
        try {
          ...
          console.log(
            `Using size-only fingerprint for ${path.basename(filePath)}: ${sizeOnlyFingerprint}`
          );
          return sizeOnlyFingerprint;
        } catch {
          throw new Error(
            `All fingerprinting methods failed: ${error.message}`
          );
        }
      }
    }
      ...
      if (files.length > 1) {
        ...
        this.duplicateGroups.push({
          ...
          avgBitrate: Math.round(
            files.reduce((sum, f) => sum + f.bitrate, 0) / files.length
          ),
        });
      }
    }
...
ipcMain.handle('analyze-directory', async (event, dirPath) => {
  try {
    ...
    try {
      ...
      return { success: false, error: 'Directory not accessible' };
    }

    const dedupe = new AudioDedupe();
    return await dedupe.analyzeDirectory(resolvedDir);
  } catch (error) {
...
ipcMain.handle('delete-files', async (event, filePaths) => {
  ...
  if (!lastScannedDir) {
    return filePaths.map(p => ({ path: p, success: false, error: 'No scan has been performed' }));
  }

  // Validate files exist before deletion
  for (const filePath of filePaths) {
    // Path traversal protection: only allow files within the scanned directory
    const resolvedPath = path.resolve(filePath);
    const resolvedScanDir = path.resolve(lastScannedDir);
    if (!resolvedPath.startsWith(resolvedScanDir + path.sep) && resolvedPath !== resolvedScanDir) {
      results.push({ path: filePath, success: false, error: 'File outside scan directory' });
      continue;
    }
...
ipcMain.handle('play-audio-file', async (event, filePath) => {
  try {
    // Path traversal protection: only allow files within the scanned directory
    if (!lastScannedDir) {
      return { success: false, error: 'No scan directory set' };
    }
    const resolvedPath = path.resolve(filePath);
    const resolvedScanDir = path.resolve(lastScannedDir);
    if (!resolvedPath.startsWith(resolvedScanDir + path.sep)) {
      return { success: false, error: 'Path outside scanned directory' };
    }

    // Read file as buffer for secure audio playback
    const buffer = await fs.readFile(resolvedPath);

    // Convert buffer to base64 for renderer use
    ...
    console.error(`Audio playback error for ${filePath}:`, error);
