...
This document provides a complete reference for AudioDUPER's internal APIs, including main process functions, IPC communication, and audio processing APIs.
...
Extracts full metadata from an audio file.
...
Errors are caught with standard try/catch and propagated via IPC as plain objects with `message` and `details` fields. No custom error classes are used.
...
try {
  ...
  // Error sent to renderer via 'app-error' event
}
```
