...
Customizable quality preferences are planned for a future release.
...
**A:** No, AudioDUPER only reads files for analysis. It never modifies original audio files or metadata. When you choose to delete files, they are permanently deleted using `fs.unlink()`. Make sure you have backups of important files before deleting.

## ⚙️ Settings and Configuration

### Q: Can I customize the quality ranking?

**A:** Not yet. Customizable quality ranking settings are planned for v1.1. Currently, AudioDUPER uses a built-in algorithm that considers bitrate, format, and file size.
...
**A:** Keyboard shortcuts are planned for a future release. Currently, all actions are performed through the UI buttons.

### Q: Can I save scan results?

**A:** Export to CSV/JSON and session saving are planned for a future release. Currently, results are displayed in the app during the active session.
...
- **No data collection**: Metadata is extracted for analysis purposes only and is not sent anywhere
- **Permanent deletion**: Files are deleted with `fs.unlink()` -- make sure to keep backups
...
- **[Quick Start Guide](QUICK_START.md)**: Full usage documentation
...
- **[GitHub Repository](https://github.com/sanchez314c/audio-duper)**: Source code and issues
